...
class GatedSelfAttentionDense(nn.Module):
    ...
    def forward(self, x, instance_options={}):
        ...
        attention_output = self.attn(  self.norm1(torch.cat([x,objs],dim=1)), instance_options=instance_options)
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self, x, context, transformer_options={}): 
        ...
        # if 'fusers' in transformer_options and transformer_options['idx'] in transformer_options['fusers']:
        block_type = transformer_options['block_type']
        idx = transformer_options['idx']
        x = transformer_options['fusers'][block_type][idx](x, transformer_options)
        # x = self.fuser(x, transformer_options)
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context, transformer_options={}): # objs, grounding_input=None, drop_box_mask=False):
        b, c, h, w = x.shape
        x_in = x
        x = self.norm(x)
        x = self.proj_in(x)
        ...
        for block in self.transformer_blocks:
            x = block(x, context, transformer_options)
