...
class CSDPredictor(nn.Module):
    ...
    def sampler_euler_function(self, model, latents, sigmas, extra_args=None, callback=None, disable=None, s_churn=0.0, s_tmin=0.0, s_tmax=float('inf'), s_noise=1.):
        ...
        noise_sampler = get_brownian_noise_sampler(src_latents, sigmas, seed=extra_args['seed'])
        ...
        for i in trange(len(sigmas) - 1):
            ...
            with torch.no_grad():
                ...
                src_noise_pred = calc_cond_uncond_batch(model, src_cond, src_uncond, src_latents_noisy, timestep, model_options)

                # TGT
                tgt_latents_noisy = ddpm_add_noise(tgt_latents, sigma, noise)
                tgt_noise_pred = calc_cond_uncond_batch(model, cond, uncond, tgt_latents_noisy, timestep, model_options)
...
def sample_sample(model, noise, steps, cfg, sampler_name, scheduler, positive, negative, src_positive, src_negative, latent_image, denoise=1.0, disable_noise=False, start_step=None, last_step=None, force_full_denoise=False, noise_mask=None, sigmas=None, callback=None, disable_pbar=False, seed=None):
    ...
    comfy.sample.cleanup_additional_models(set(comfy.sample.get_models_from_cond(src_positive_copy, "control") + comfy.sample.get_models_from_cond(src_negative_copy, "control")))
    return samples


def common_ksampler(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, src_positive, src_negative, latent, denoise=1.0, disable_noise=True, start_step=None, last_step=None, force_full_denoise=False):
...
class KSamplerCSD():
  ...
  def INPUT_TYPES(s):
      return {"required":
                  ...
                  "image_cfg": ("FLOAT", {"default": 1.50, "min": 0.00, "max": 100.0, "step":0.01, "round": 0.01}),
  ...
  @torch.no_grad()
  def sample(self, model, noise_seed, add_noise, steps, cfg, image_cfg, sampler_name, scheduler, positive, negative, src_positive, src_negative, latent_image, start_at_step, end_at_step, learning_rate, weight_decay, decay_iterations, decay_rates):
    global image_guidance_scale, lr, wd, decay_iters, decay_rate
